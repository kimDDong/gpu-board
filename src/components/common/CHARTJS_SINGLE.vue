...
const renderChartJS = () => {
  ...
  chartInstance.value = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        title: { display: true, text: 'SINGLE-CHART-JS', font: { size: 16 } },
...
watch([() => props.startDate, () => props.endDate], async ([newStartDate, newEndDate], [oldStartDate, oldEndDate]) => {
  await fetch(); // 새 날짜로 데이터 가져오기
  renderChartJS();      // 차트 다시 그리기
});
