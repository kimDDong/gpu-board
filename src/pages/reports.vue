...
async function fetchAll() {
  ...
  sysinfo.value = (await axios.get('https://gpu-board.onrender.com/api/report/sysinfo', { params: { end } })).data
  const usage = (await axios.get('https://gpu-board.onrender.com/api/report/total_usage', { params: { start, end } })).data
  totalUsage.value = usage
  usageLoaded.value = true
  await fetchStatus()
  userRank.value = (await axios.get('https://gpu-board.onrender.com/api/report/rank', { params: { start, end } })).data
...
async function fetchIndividualDetails() {
  ...
  if (selectedGpu.value) {
    gpuDetail.value = (await axios.get('https://gpu-board.onrender.com/api/report/individual_usage', { params: { type: "GPU", name: selectedGpu.value, start, end } })).data
  }
  if (selectedCpu.value) {
    cpuDetail.value = (await axios.get('https://gpu-board.onrender.com/api/report/individual_usage', { params: { type: "CPU", name: selectedCpu.value, start, end } })).data
  }
  if (selectedMemory.value) {
    memoryDetail.value = (await axios.get('https://gpu-board.onrender.com/api/report/individual_usage', { params: { type: "Memory", name: selectedMemory.value, start, end } })).data
  }
  // 선택된 GPU 온도들 동시 갱신
  for (const gpu of selectedGpuTemps.value) {
    if (!gpuTempSeries.value[gpu]) {
      const res = await axios.get('https://gpu-board.onrender.com/api/report/gpu_temp_series', { params: { name: gpu, start, end } })
